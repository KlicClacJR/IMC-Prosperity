...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        memory = self.load_memory(getattr(state, "traderData", ""))

        raw_positions = getattr(state, "position", {})
        raw_order_depths = getattr(state, "order_depths", {})
        positions = raw_positions if isinstance(raw_positions, dict) else {}
        order_depths = raw_order_depths if isinstance(raw_order_depths, dict) else {}
    ...
    def load_memory(self, trader_data: str):
        if trader_data and isinstance(trader_data, str):
            try:
